...
void naive_gaussian_blur_5_expand(unsigned char *src,int w,int h)
{
    for(int j=2; j<h-7; j+=5) {
        for(int i=2; i<w-7; i+=5) {
            int sum = 0;
            const int temp11 = src[(j)*w+(i)],temp12 = src[(j)*w+(i+1)],temp13 = src[(j)*w+(i+2)],temp14 = src[(j)*w+(i+3)],temp15 = src[(j)*w+(i+4)],
                      temp21 = src[(j+1)*w+(i)],temp22 = src[(j+1)*w+(i+1)],temp23 = src[(j+1)*w+(i+2)],temp24 = src[(j+1)*w+(i+3)],temp25 = src[(j+1)*w+(i+4)],
                      temp31 = src[(j+2)*w+(i)],temp32 = src[(j+2)*w+(i+1)],temp33 = src[(j+2)*w+(i+2)],temp34 = src[(j+2)*w+(i+3)],temp35 = src[(j+2)*w+(i+4)],
                      temp41 = src[(j+3)*w+(i)],temp42 = src[(j+3)*w+(i+1)],temp43 = src[(j+3)*w+(i+2)],temp44 = src[(j+3)*w+(i+3)],temp45 = src[(j+3)*w+(i+4)],
            ...
            src[(j+2)*w+i-1] += (temp11*gaussian55[21]+temp12*gaussian55[20]
                                 ...
                                 +temp51*gaussian55[1]+temp52*gaussian55[0])/deno55;
            sum = (temp11*gaussian55[22]+temp12*gaussian55[21]+temp13*gaussian55[20]
                   +temp21*gaussian55[17]+temp22*gaussian55[16]+temp23*gaussian55[15]
            ...
            sum = (temp21*gaussian55[22]+temp22*gaussian55[21]+temp23*gaussian55[20]
