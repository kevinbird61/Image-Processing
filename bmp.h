...
#pragma pack( push, 1 )                     //struct alignment BMPHEADER
typedef struct tagBITMAPFILEHEADER {        //(14bytes)
    WORD bfType;                        //(2bytes)         File type, in BMP case, (0x424D)
...
typedef struct tagBITMAPINFOHEADER {        //(40bytes)
    DWORD biSize;                       //(4bytes)        After Windows 3.X, It is always 40, which is the structure size of BITMAPINFOHEADER
