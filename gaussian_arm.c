...
void neon_gaussian_blur_5_tri(unsigned char *src,int w,int h)
{
    ...
    for(int i=0; i<w-2; i++) {
        for(int j=0; j<h-2; j++) {
            ...
            //
            uint8x16x2_t V0 = vzipq_u8(L0,vk0);
            uint8x16x2_t V1 = vzipq_u8(L1,vk0);
            uint8x16x2_t V2 = vzipq_u8(L2,vk0);
            uint8x16x2_t V3 = vzipq_u8(L3,vk0);
            uint8x16x2_t V4 = vzipq_u8(L4,vk0);
